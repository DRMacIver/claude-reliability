...
mod tests {
    ...
    #[test]
    fn test_is_delete_command_rm() {
        assert!(is_delete_command("rm .claude/reliability-config.yaml"));
        assert!(is_delete_command("rm -f .claude/reliability-config.yaml"));
        assert!(is_delete_command("rm -rf .claude/reliability-config.yaml"));
    }

    #[test]
    fn test_is_delete_command_not_config() {
        assert!(!is_delete_command("rm other-file.txt"));
        assert!(!is_delete_command("rm -rf target/"));
    }

    #[test]
    fn test_is_delete_command_redirect() {
        assert!(is_delete_command("echo '' > .claude/reliability-config.yaml"));
    }
